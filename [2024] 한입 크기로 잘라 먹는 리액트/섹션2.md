...
- 타임스탬프
  ...
  - 일밙적으로, getTime() 메서드를 사용하여 타임스탬프를 얻는다.
- 시간 요소들을 추출하는 방법
  - 년 : getFullYear()
  - 월 : getMonth()
  - 일 : getDate()
...
- 비동기 : 동기적이지 않으며, 작업을 순서대로 처리하지 않는다.
  ...
  - 자바스크립트의 비동기 처리
    ...
    - JavaScript Engine -> setTimeout -> WebAPIs 타이머-> 콜백함수 -> JavaScript Engine -> 콜백함수 실행.
...
- Promise 객체
  ...
  - executor : Promise 객체 내 비동기 작업으로 콜백함수 내 정의
  - resolve,reject : 콜백함수의 인수로 받는, reolve는 성공, reject는 실패를 의미한다.
